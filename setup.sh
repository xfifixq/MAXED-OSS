...
# Invoice Ninja APP_KEY and admin password
set_env "INVOICENINJA_APP_KEY" "base64:$(openssl rand -base64 32)"
set_env "INVOICENINJA_ADMIN_EMAIL" "admin@maxed.life"
set_env "INVOICENINJA_ADMIN_PASSWORD" "$ADMIN_PASS"
...
echo ""
echo "  Custom Apps:"
echo "    Dashboard        https://app.maxed.life     (CPA admin panel)"
echo "    Client Portal    https://portal.maxed.life  (client-facing)"
echo "    OpenCPA Site     https://opencpa.maxed.life  (marketing)"
echo "    API              https://api.maxed.life      (platform API)"
echo ""
echo "  Service UIs (via Docker + nginx):"
echo "    Paperless-ngx   https://docs.maxed.life     (admin / $ADMIN_PASS)"
echo "    n8n             https://flow.maxed.life     (admin / $ADMIN_PASS)"
echo "    Invoice Ninja   https://billing.maxed.life  (admin@maxed.life / $ADMIN_PASS)"
echo "    Bigcapital      https://books.maxed.life    (create account on first visit)"
echo "    Metabase        https://reports.maxed.life  (create account on first visit)"
echo "    DocuSeal        https://sign.maxed.life     (create account on first visit)"
echo "    Twenty CRM      https://crm.maxed.life      (create account on first visit)"
echo "    Mattermost      https://chat.maxed.life     (create account on first visit)"
echo "    Kimai           https://time.maxed.life     (admin@maxed.life / $ADMIN_PASS)"
echo ""
echo "  Next steps:"
echo "    1. Point *.maxed.life DNS to your server IP"
echo "    2. Set up SSL: certbot certonly --manual -d '*.maxed.life' -d maxed.life"
echo "    3. Import n8n workflows from platform/n8n-workflows/"
echo "    4. Configure Metabase (connect to PostgreSQL: maxed_unified)"
echo "    5. Set up Bigcapital org and chart of accounts"
echo ""
